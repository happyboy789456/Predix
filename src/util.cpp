...
const char * const BITCOIN_CONF_FILENAME = "Predix.conf";
const char * const BITCOIN_PID_FILENAME = "Predixd.pid";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Predix
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Predix
    // Mac: ~/Library/Application Support/Predix
    // Unix: ~/.Predix
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PDCC";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PDCC";
#else
    // Unix
    return pathRet / ".PDCC";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Predix Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Predix Core developers";
    }
